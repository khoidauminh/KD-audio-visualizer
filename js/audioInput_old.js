...
function processAudio(canvas, stream, buffer, display) {
    ...
    setInterval(() => { flip = visualize(canvas, forceMono(bufferL, bufferR), flip, display.d); }, 18);
...
async function initAudioInput(canvas, constraints, buffer, display) {
  let stream = null;
  ...
  try {
    ...
    processAudio(canvas, stream, buffer, display);
